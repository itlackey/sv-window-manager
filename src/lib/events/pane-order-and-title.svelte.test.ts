...
import { onpaneorderchanged, onpanetitlechanged, removeEventHandler } from './dispatcher.js';
...
describe('Pane order-changed and title-changed events', () => {
	...
	it('emits onpaneorderchanged for both panes when swapping via center drop', async () => {
		...
		for (const e of events) {
			...
			expect(
				typeof e.context?.previousIndex === 'number' || e.context?.previousIndex === undefined
			).toBe(true);
		}
	});
