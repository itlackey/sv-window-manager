...
test.describe('FitContainer Debug - Initial Load Investigation', () => {
  test('DEBUG: Capture Initial Page Load State', async ({ page }) => {
    ...
    const fitContainerLogs = consoleLogs.filter(log =>
  ...
  test('DEBUG: Compare State After Add Pane', async ({ page }) => {
    ...
    const fitContainerLogsAfter = consoleLogs.filter(log =>
      log.text.includes('fitContainer') ||
      log.text.includes('Set dimensions')
    );
    ...
    console.log('BW Container changed:',
      bwBoxBefore?.width !== bwBoxAfter?.width ||
      bwBoxBefore?.height !== bwBoxAfter?.height
    );
  });
  ...
  test('DEBUG: Test Container Resize Behavior', async ({ page }) => {
    ...
    const resizeLogs = consoleLogs.filter(log =>
      log.text.includes('ResizeObserver') || log.text.includes('fitContainer')
    );
